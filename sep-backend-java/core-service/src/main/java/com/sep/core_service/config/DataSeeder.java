...
public class DataSeeder implements CommandLineRunner {
    ...
    public void run(String... args) throws Exception {
        ...
        // Tạo sinh viên có profile đầy đủ
        User sv01 = createUserIfNotFound("sv01", "Vũ Trí Dũng", "sv1@sep.com", studentRole, "022205001700", "23/11/2005", "Quảng Ninh", "Nam");
        ...
        // Nạp điểm mẫu cho sv01
        seedDynamicGrades(sv01);
    }

    private void seedDynamicGrades(User user) {
        System.out.println("⏳ Đang nạp hệ thống điểm chuẩn...");

        Student student = studentRepository.findById(user.getId()).orElseGet(() -> {
            Student s = new Student();
            s.setUser(user);
            s.setStudentCode(user.getStudentCode());
            s.setEnrollmentYear(2023);
            s.setAcademicStatus("STUDYING");
            return studentRepository.save(s);
        });

        GradeComponent qtComponent = createComponentIfNotFound("ĐIỂM QUÁ TRÌNH", 0.4);
        GradeComponent thiComponent = createComponentIfNotFound("ĐIỂM THI CUỐI KỲ", 0.6);

        subjectRepository.findBySubjectCode("0101124101").ifPresent(subject -> {
            createMockClassAndGrade(subject, student, "HK1", "2024-2025", qtComponent, thiComponent, 8.5, 9.0);
        });
        
        subjectRepository.findBySubjectCode("0101001213").ifPresent(subject -> {
            createMockClassAndGrade(subject, student, "HK1", "2024-2025", qtComponent, thiComponent, 7.0, 6.5);
        });
    }

    private GradeComponent createComponentIfNotFound(String name, double weight) {
        GradeComponent gc = new GradeComponent();
        gc.setName(name);
        gc.setWeight(weight);
        return componentRepository.save(gc);
    }

    private void createMockClassAndGrade(Subject subject, Student student, String semester, String year, 
                                        GradeComponent qt, GradeComponent thi, double diemQT, double diemThi) {
        CourseClass cc = new CourseClass();
        cc.setSubject(subject);
        cc.setSemester(semester);
        cc.setAcademicYear(year);
        cc = courseClassRepository.save(cc);

        Enrollment enrollment = new Enrollment();
        enrollment.setStudent(student);
        enrollment.setCourseClass(cc);
        enrollment.setEnrollmentDate(LocalDate.now());
        enrollment = enrollmentRepository.save(enrollment);

        GradeScore score1 = new GradeScore();
        score1.setEnrollment(enrollment);
        score1.setComponent(qt);
        score1.setScore(diemQT);
        gradeScoreRepository.save(score1);

        GradeScore score2 = new GradeScore();
        score2.setEnrollment(enrollment);
        score2.setComponent(thi);
        score2.setScore(diemThi);
        gradeScoreRepository.save(score2);
    }
    ...
    private void seedSubjectRelationsPhase2() {
        System.out.println("🔗 [Phase 2] Đang thiết lập các Môn học trước...");
        addPreviousSubject("0101121000", "0101124101");
    }
    ...
    // 🔥 HÀM 4 THAM SỐ (Cho Admin, Giảng viên)
    ...
    // 🔥 HÀM 8 THAM SỐ (Cho Sinh viên có Profile đầy đủ)
...
//         // Nạp dữ liệu 2 bước: 1. Tạo môn -> 2. Gắn điều kiện
//         seedSubjectsPhase1();
//         seedSubjectRelationsPhase2();
//     }
